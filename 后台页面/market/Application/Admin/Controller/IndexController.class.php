...
class IndexController extends Controller {
 ...
 //    			);
	...
	//   	$addanswer->add($data);
	//   	$t=I('post.id');
	//   	$result=$addanswer->where("idq=$t")->count();
	//   	if($result>0){
	//   		$this->success("添加成功！");
	//   	}
	//   	else{
	//   		$this->error($addanswer->getError());
	//   	}
	//   }
 //    	else{
 //    		$id=$_GET['id'];
 //    		// echo $id;exit();
 //    		$result=M('question');
 //    		$q=$result->where("id=$id")->select();
 //    		$this->assign('q',$q);

 //    		$a=M('answer')->where("idq=$id and usertype='用户'")->select();
 //    		$this->assign('a',$a);
 //    		$b=M('answer')->where("idq=$id and usertype='管理员'")->select();
 //    		$this->assign('b',$b);
	// 		$this->display();
 //    	}
 //    }

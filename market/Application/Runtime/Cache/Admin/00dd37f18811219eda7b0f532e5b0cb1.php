...
<form method="post" >
	<div class="searchBar">
			<label>用户姓名：</label>
			<input type="text" id="text" name="sousuob" value="请输入搜索内容"/>
		<div class="subBar">
			<ul>
				<li>
				<a href="#" class="button" target='navTab' class='add button' name='submitb' id="submitb"><span name='button'>搜索</span></a>
				</li>
			</ul>
		</div>
	</div>
		...
		<tbody>
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><tr target="sid_user" rel="<?php echo ($d["id"]); ?>">
					...
					<td>
						...
						<a href="/index.php/Admin/Yhgl/recover?id=<?php echo ($d["id"]); ?>" target="navTab" class="add button"><span>恢复</span></a>
...
<script>
	var submit=document.getElementsByName('submitb')[0];
	var sousuo=document.getElementsByName('sousuob')[0];

<?php if (!defined('THINK_PATH')) exit();?><form id="pagerForm" action="/demo/index.php/Admin/Goods" method="post">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="/demo/index.php/Admin/Goods/add" target="navTab"><span>添加</span></a></li>
			<li><a class="delete" href="/demo/index.php/Admin/Goods/destroy/id/{sid_user}" target="ajaxTodo" title="确定要删除吗？" warn="请选择一个用户"><span>删除</span></a></li>
			<li><a class="edit" href="/demo/index.php/Admin/Goods/edit?id={sid_user}" target="navTab" warn="请选择一个用户"><span>修改</span></a></li>		
			<li><a class="icon" href="/demo/index.php/Admin/Goods/lists/id/{sid_user}" target="dialog"><span>详情</span></a></li>			
		</ul>
	</div>
	<table class="list" width="98%" targetType="navTab" asc="asc" desc="desc" layoutH="126" >
		...
		<tbody>
			<?php if(is_array($results)): $i = 0; $__LIST__ = $results;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><tr target="sid_user" rel="<?php echo ($d["id"]); ?>">
					...
					<td >
					<?php if(is_array($d['image'])): $i = 0; $__LIST__ = $d['image'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$img): $mod = ($i % 2 );++$i;?><img src="/demo/Public/<?php echo ($img); ?>" style="height: 29px;width: 29px;"/><?php endforeach; endif; else: echo "" ;endif; ?>						
					</td>
...
<script>
	...
	var data="/demo/index.php/Admin/Goods/index";
	sousuo.onclick=function(){
		this.value='';
	}
	submit.onmouseover=function(){	
	data="/demo/index.php/Admin/Goods/search?condition="+sousuo.value;
	this.href=data;
	}
</script>
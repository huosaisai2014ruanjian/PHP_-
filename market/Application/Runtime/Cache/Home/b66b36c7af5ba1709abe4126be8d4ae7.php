<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html>

<head>
    <title>认证成功</title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <!--IE8-->
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no">
    <!--屏幕自适应-->
    <link rel="stylesheet" type="text/css" href="/market/Public/lib/weui.min.css">
    <link rel="stylesheet" type="text/css" href="/market/Public/css/jquery-weui.css">

    <body>
        <div class="weui_msg">
            <div style="margin-top:30%;" class="weui_icon_area"><i class="weui_icon_success weui_icon_msg"></i></div>
            <div class="weui_text_area">
                <h2 class="weui_msg_title">提交成功</h2>
                <p class="weui_msg_desc">请等待审核通过</p>
            </div>
            <div class="weui_opr_area">
                <p class="weui_btn_area">
                    <a href="/market/index.php/Home/Personal/percenter" class="weui_btn weui_btn_primary">确定</a>
                    <!-- <a href="javascript:;" class="weui_btn weui_btn_default">取消</a> -->
                </p>
            </div>
            <!-- 
  <div class="weui_extra_area">
    <a href="">查看详情</a>
  </div> -->
        </div>
    </body>
    <!-- <script type="text/javascript" src="/market/Public/lib/jquery-2.1.4.js"></script> -->
    <script type="text/javascript" src="/market/Public/js/jquery-weui.min.js"></script>
    </script>

</html>